...
| `event`      | Events             | `title`, `dateTime`, `endDate`, `location`, `tags`, `image`, `imgpos`, `url`, `url-button-label`, `data_luma_event_id`, `partnerEvent`, `partnerOrganization`, `series`, `seriesPart`, `seriesTotal` — **files live in `src/content/meetups/`** (folder name differs from collection name) |
...
└── newsletter/    # SinglePost.astro (renders newsletter issue HTML), ExecutiveBoard.astro (live board grid), TableOfContents.astro (styled TOC box), HighlightBox.astro / SponsorTierList.astro / CTABox.astro (styled callout boxes, see edit-newsletter skill)
```
...
- `events/Signup.astro` — renders the registration button on event pages. Accepts `url`, `urlButtonLabel`, `data_luma_event_id`, and `slug` props. Automatically appends `utm_source=boston-wib&utm_medium=event-page&utm_campaign=<slug>` to the registration URL — **do not manually add UTM params to the `url` field in event frontmatter**. The slug is passed from `[...slug].astro` as `meeting.data.slug ?? meeting.id`. `urlButtonLabel` (sourced from the event frontmatter's `url-button-label` field) overrides the default "Sign Up" button text — use it when `url` isn't a registration link (e.g. `'Learn More'` for a link to a partner org's own event page). It only affects the plain-`url` button; the two Lu.ma-checkout button labels ("Register for Event" / "Register for In-Person Site") are unaffected.
- **Multi-part event series** (`series`/`seriesPart`/`seriesTotal` fields) — for something like a 4-part workshop, each part is its own independent event file in `src/content/meetups/` with all three fields set explicitly (no shared series collection, no auto-counting across the collection — `content.config.ts`'s event schema `.refine()`s that all three are set together and `seriesPart <= seriesTotal`). `EventLayout.astro` renders a "`series` · Part `seriesPart` of `seriesTotal`" pill above the title on the individual event page; `EventsTable.astro` renders a matching "(Part `seriesPart` of `seriesTotal`)" annotation next to the title on `/events` listing cards (same inline-span pattern used for the existing "(Partner Event)" annotation). The past-events archive (`src/pages/events/archive/index.astro`) goes further: it groups past events sharing the same `series` name server-side and, once **every** part (`group.length === seriesTotal`) has passed, collapses all of them into a single summary card — title = the series name, date shown as a range (Part 1's start → the last part's end), image/link = Part 1's, tags = the union of all parts' tags — flagged internally via a synthesized `isSeriesSummary: true` entry (see the `toClientEvent` helper and the `seriesGroups` loop in that file). While a series is still in progress (fewer than `seriesTotal` parts have passed), its completed parts keep showing individually with the same "(Part X of Y)" annotation as `EventsTable.astro`, and only collapse once the final part also passes. A `seriesTotal` mismatch across a series' files, or duplicate/missing `seriesPart` numbers, is treated as a content-authoring error with no extra validation beyond the schema's `.refine()` — not guarded against at render time.
...
26. **Hand-written multi-line HTML in `.mdx` bodies can silently mis-parent text into stray `<p>` tags** — MDX's markdown parser (remark) decides paragraph boundaries by line position, not by JSX tag nesting. A block like `<div><p>text <a>link</a> more text</p></div>`, when its children span multiple source lines, can get its text re-parented into sibling `<p>` tags (sometimes empty ones) instead of staying inside the `<p>` you wrote — this drops that `<p>`'s class and can break inline flow (e.g. a link ending up outside its sentence). `astro build` does not error on this; it only shows up by inspecting the built HTML. This bit an early draft of the newsletter callout box components (`HighlightBox.astro`, `SponsorTierList.astro`, `CTABox.astro` — see Local Norm 23 and the `edit-newsletter` skill): `astro build` succeeded, but the built HTML had unstyled empty `<p>` tags. The fix is to pass such content through a component **prop** (a string rendered via `set:html`, or a structured data array) rather than as literal JSX children in the `.mdx` file — remark never touches prop values, only literal children. If you hand-write multi-line raw HTML directly in a `.mdx` body (newsletter or blog post) and something renders unstyled or oddly line-broken, check the built HTML for unexpected `<p>` tags before assuming it's a CSS issue.
